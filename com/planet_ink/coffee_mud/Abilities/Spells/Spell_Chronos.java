...
*/
public class Spell_Chronos extends Spell
{

	@Override
	public String ID()
	{
		return "Spell_Chronos";
	}

	private final static String localizedName = CMLib.lang().L("Chronos");
	...
	private final static String localizedStaticDisplay = CMLib.lang().L("^C(^cChronos^C)^N");
	...
	public void unInvoke()
	{
		...
		if(super.canBeUninvoked())
			mob.tell(L("Your Chronos wears away."));
	}
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		{
			invoker=mob;
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),L(auto?"<T-NAME> seem(s) to look blank for a moment.":"^S<S-NAME> seem(s) to look blank for a moment.^?"));
