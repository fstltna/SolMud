...
*/
public class Fighter_Doorbash extends FighterSkill
{
	@Override
	public String ID()
	{
		return "Fighter_Doorbash";
	}

	private final static String	localizedName	= CMLib.lang().L("Door Bash");
	...
	private static final String[]	triggerStrings	= I(new String[] { "Doorbash" });
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		{
			mob.tell(L("You can't ram @x1!",openThis.name()));
			return false;
		}
		...
		if(!success)
			beneficialVisualFizzle(mob,openThis,auto?L("Nothing happens to @x1.",openThis.name()):L("<S-NAME> reel(s) back and ram(s) @x1 with a shoulder but nothing happens.",openThis.name()));
		else
		{
			CMMsg msg=CMClass.getMsg(mob,openThis,this,somaticCastCode(mob,openThis,auto),(auto?L("@x1's lock break(s) open!",openThis.name()):L("^S<S-NAME> <S-NAME> reel(s) back and ram(s) <T-NAMESELF> with a shoulder!^?"))+CMLib.protocol().msp("doorbash.wav",10));
			if(R.okMessage(mob,msg))
			{
				...
				for(int a=0;a<openThis.numEffects();a++)
				{
					...
					if((A!=null)&&(A.ID().equalsIgnoreCase("Spell_WizardLock")))
					{
						...
						{
							A.unInvoke();
							R.show(mob,null,openThis,CMMsg.MSG_OK_VISUAL,L("A spell around <O-NAME> seems to break."));
							break;
						}
					}
				}
				...
				msg=CMClass.getMsg(mob,openThis,null,CMMsg.MSG_OPEN,L("<T-NAME> swings open!"));
				CMLib.utensils().roomAffectFully(msg,R,dirCode);
			}
		}
