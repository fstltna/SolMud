...
*/
public class Echo extends StdCommand
{
	public Echo()
	{
	}

	private final String[] access=I(new String[]{"ECHO"});
	@Override
		...
		throws java.io.IOException
	{
		...
		if(commands.size()<2)
		{
			if((commands.size()>0)&&(commands.get(0).equalsIgnoreCase(",")))
				CMLib.commands().postCommandFail(mob, commands, L(" ECHO which social? Try SOCIALS."));
			else
				CMLib.commands().postCommandFail(mob, commands, L(" ECHO what?"));
			return false;
		}
		...
		final String emote="^E"+combinedCommands+" ^c[^C<S-NAME>^c] ^?";
